    ...
    fetch(urlBase, { method: "GET" })
      ...
      .then((listaAgendamentos) => {
        if (Array.isArray(listaAgendamentos)) {
          const resultadoBusca = listaAgendamentos.filter((agendamento) =>
            agendamento.campo.toLowerCase().includes(termoBusca)
          );
        ...
        <Row>
          <Col>
            <FormControl
              ...
              id="termoBusca"
              onChange={filtrar}
              placeholder="Digite o CAMPO para ver seus agendamentos"
            />
          </Col>
          <Col></Col>
        </Row>
